...
import { partitionPushChanges, type PushChange, type SyncCapabilities } from "../../src/agentwiki/protocol";
...
describe("partitionPushChanges", () => {
  it("uses deterministic ordering and both item and byte limits", async () => {
    const changes: PushChange[] = ["c", "a", "b"].map((pageId) => ({ operation: "archive", pageId, previousPath: `${pageId}.md` }));
    const batches = await partitionPushChanges(changes, capabilities);
    expect(batches.map((batch) => batch.changes.map((change) => change.pageId))).toEqual([["a", "b"], ["c"]]);
  ...
  it("rejects a single change that cannot fit a legal batch", async () => {
    ...
    await expect(partitionPushChanges([{ operation: "upsert", pageId: "a", path: "a.md", title: "a", body, contentHash: "0".repeat(64) }], capabilities)).rejects.toThrow(/PAGE_TOO_LARGE/);
  });
});
