...
describe("PullTransaction", () => {
  ...
  it("recognizes a fully materialized rename after temporary cleanup as committed",async()=>{const vault=new MemoryVault({"A.md":"old"});const control=new MemoryControlStore();const transaction=new PullTransaction(vault,control,".agentwiki/tx/final-window");await transaction.prepare([{kind:"rename",fromPath:"A.md",path:"B.md",body:"new"}],1,"tx-final");const original=new TextEncoder().encode("old");const result=new TextEncoder().encode("new");const originalHash=await sha256Hex(original);const resultHash=await sha256Hex(result);await transaction.replaceForRecoveryTest(raw=>{raw.state="applying";raw.temporaryPaths=[{original:"A.md",temporary:"A.md.agentwiki-tmp-0",expectedHash:originalHash}];raw.materialized=[{path:"B.md",resultHash,expectedHash:null}];});await vault.rename("A.md","A.md.agentwiki-tmp-0");await vault.write("B.md",result);await vault.remove("A.md.agentwiki-tmp-0");await transaction.recover();expect(vault.text("B.md")).toBe("new");expect((await transaction.inspect())?.state).toBe("committed");});
});
