...
describe("AgentWiki connection", () => {
  ...
  it("stores a credential before exchange and activates only after verification", async () => {
    ...
    expect(http.calls.map((call) => call.path)).toEqual(["/api/integrations/obsidian/exchange", "/api/integrations/obsidian/credentials/current", "/api/integrations/obsidian/credentials/current/activate"]);
  });
});
