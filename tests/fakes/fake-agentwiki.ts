import { canonicalBytes, contentHash, revisionContentHash, type PushBatch, type SyncCapabilities, type SyncPage } from "../../src/agentwiki/protocol";
import type { FinalizeResult, PushRemotePort, PushSessionInfo } from "../../src/ports/push-remote";

interface Session { info: PushSessionInfo; baseRevision: string; batches: Map<number, PushBatch> }
export class FakeAgentWiki implements PushRemotePort {
  ...
  async createSession(input: { baseRevision: string }): Promise<PushSessionInfo> {
    ...
    const info: PushSessionInfo = { sessionId, status: "uploading", expiresAt: "2099-01-01T00:00:00.000Z", capabilities: this.capabilities, result: null };
    this.sessions.set(sessionId, { info, baseRevision: input.baseRevision, batches: new Map() }); return info;
  }
  async uploadBatch(sessionId: string, batch: PushBatch): Promise<{ receipt: string }> { this.sessions.get(sessionId)!.batches.set(batch.batchIndex, batch); return { receipt: `r-${batch.batchIndex}` }; }
  async finalize(sessionId: string): Promise<FinalizeResult> {
    if (!this.canPublish) throw new Error("SPACE_READ_ONLY");
    const session = this.sessions.get(sessionId)!;
    if (session.baseRevision !== String(this.revision)) throw new Error("BASE_STALE");
    for (const batch of [...session.batches.values()].sort((a, b) => a.batchIndex - b.batchIndex)) for (const change of batch.changes) {
      if (change.operation === "archive") this.pages.delete(change.pageId);
      else this.pages.set(change.pageId, { pageId: change.pageId, path: change.path, title: change.title, body: change.body, contentHash: await contentHash(change.body), updatedAt: "2026-08-14T00:00:00.000Z" });
    }
    this.revision += 1;
    const manifest = { pages: [...this.pages.values()].map((page) => ({ pageId: page.pageId, path: page.path, title: page.title, contentHash: page.contentHash })) };
  ...
  async advanceEmptyRevision():Promise<void>{this.revision+=1;}
