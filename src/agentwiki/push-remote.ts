...
import { CreatePushSessionRequestSchema, CreatePushSessionResponseSchema, FinalizePushRequestSchema, FinalizeResultSchema, parseCapabilities, PushReceiptSchema, PushSessionStatusResponseSchema, type PushBatch } from "./protocol";
...
export class AgentWikiPushRemote implements PushRemotePort {
  ...
  async createSession(input: Parameters<PushRemotePort["createSession"]>[0]): Promise<PushSessionInfo> { const body=CreatePushSessionRequestSchema.parse(input);const value = CreatePushSessionResponseSchema.parse((await this.client.raw("POST", `/api/sync/v1/spaces/${encodeURIComponent(this.spaceId)}/push-sessions`, body)).json); return { ...value, capabilities: parseCapabilities(value.capabilities) }; }
  ...
  async finalize(sessionId: string, confirmationHash: string): Promise<FinalizeResult> { const body=FinalizePushRequestSchema.parse({confirmationHash,userConfirmed:true});return FinalizeResultSchema.parse((await this.client.raw("POST", `/api/sync/v1/spaces/${encodeURIComponent(this.spaceId)}/push-sessions/${encodeURIComponent(sessionId)}/finalize`, body)).json); }
