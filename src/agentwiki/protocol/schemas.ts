...
const CanonicalIndexSchema=z.string().regex(/^(0|[1-9][0-9]*)$/).refine(value=>Number.isSafeInteger(Number(value)));
...
export const PushSessionParamsSchema=z.object({spaceId:PublicIdSchema,sessionId:UuidSchema}).strict();
export const PushBatchParamsSchema=z.object({spaceId:PublicIdSchema,sessionId:UuidSchema,batchIndex:CanonicalIndexSchema}).strict();
export const CredentialParamsSchema=z.object({credentialId:UuidSchema}).strict();
export const InstallationParamsSchema=z.object({installationId:UuidSchema}).strict();
export const SnapshotQuerySchema=z.object({revision:z.union([z.literal("current"),PublicIdSchema]),cursor:CursorSchema.optional(),limit:CanonicalIndexSchema.refine(value=>Number(value)>=1&&Number(value)<=200).optional()}).strict();
export const DeltaQuerySchema=z.object({from:PublicIdSchema,cursor:CursorSchema.optional(),limit:CanonicalIndexSchema.refine(value=>Number(value)>=1&&Number(value)<=200).optional()}).strict();
export const CreateObsidianInstallationRequestSchema=z.object({pluginId:z.literal("agentwiki-sync"),requestedProtocolVersion:z.literal("1")}).strict();
export const ExchangeObsidianCredentialRequestSchema=z.object({code:z.string().regex(/^[A-Za-z0-9_-]{27,256}$/),exchangeId:UuidSchema,credential:z.string().regex(/^[A-Za-z0-9_-]{43}$/),deviceId:UuidSchema,deviceName:z.string().trim().min(1).max(100),vaultId:UuidSchema,pluginVersion:z.string().max(64),supportedProtocolVersions:z.array(z.string().regex(/^(0|[1-9][0-9]*)$/)).min(1).max(8).refine(values=>new Set(values).size===values.length&&values.includes("1"))}).strict();
export const ActivateCurrentObsidianCredentialRequestSchema=z.object({credentialId:UuidSchema}).strict();
export const CreatePushSessionRequestSchema=z.object({baseRevision:PublicIdSchema,idempotencyKey:UuidSchema,capabilitiesHash:HashSchema,confirmationHash:HashSchema,confirmationByteLength:z.number().int().positive().max(4_194_304),changeCount:z.number().int().min(0).max(5_000),totalBodyBytes:z.number().int().nonnegative()}).strict();
export const FinalizePushRequestSchema=z.object({confirmationHash:HashSchema,userConfirmed:z.literal(true)}).strict();
export const SyncApiErrorResponseSchema=z.object({protocolVersion:z.literal("1"),error:z.object({code:z.string(),message:z.string(),retryable:z.boolean(),details:z.record(z.string(),z.union([z.string(),z.number(),z.boolean(),z.null()])).optional()}).passthrough()}).passthrough();
...
export const FinalizeResultSchema = z.object({protocolVersion:z.literal("1").optional(), status: z.enum(["published", "noop"]), revision: PublicIdSchema, sequence: z.number().int(), publishedAt: Rfc3339Schema.nullable(), revisionContentHash: HashSchema, pageCount: DecimalCountSchema, revisionManifestByteLength: DecimalByteCountSchema, revisionBodyBytes: DecimalByteCountSchema, changeSetId: z.string().nullable() }).passthrough();
export const CreatePushSessionResponseSchema = z.object({protocolVersion:z.literal("1").optional(), sessionId: UuidSchema, status: z.enum(["uploading", "ready_to_finalize", "published", "aborted", "expired"]), expiresAt: Rfc3339Schema, capabilities: SyncCapabilitiesSchema, result: FinalizeResultSchema.nullable() }).passthrough();
export const PushSessionStatusResponseSchema = z.object({protocolVersion:z.literal("1").optional(), sessionId: UuidSchema, status: z.enum(["uploading", "ready_to_finalize", "published", "aborted", "expired"]), expiresAt: Rfc3339Schema, receivedBatchIndexes: z.array(z.number().int().nonnegative()), result: FinalizeResultSchema.nullable() }).passthrough();
