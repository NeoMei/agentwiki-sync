...
function pageBodyBytes(change: PushChange): number {
  return change.operation === "upsert" ? encoder.encode(change.body.replace(/\r\n?/g, "\n")).byteLength : 0;
}

async function materializeBatch(batchIndex: number, changes: PushChange[]): Promise<PushBatch> {
  const withoutHash = { protocolVersion: "1" as const, batchIndex, changes };
  return { ...withoutHash, batchHash: await batchHash(withoutHash) };
}

export async function partitionPushChanges(changes: PushChange[], capabilities: SyncCapabilities): Promise<PushBatch[]> {
  if (changes.length > capabilities.maxChangeCount) throw new RangeError("BATCH_TOO_LARGE");
  ...
  for (const change of sorted) {
    ...
    if (exceeds) {
      ...
      if (canonicalBytes(single).byteLength > capabilities.maxBatchBytes) throw new RangeError("BATCH_TOO_LARGE");
    } else {
      current = proposed;
    }
  }
  if (current.length > 0) batches.push(await materializeBatch(batches.length, current));
  return batches;
}
