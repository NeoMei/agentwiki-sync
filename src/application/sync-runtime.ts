...
interface BaseState { schemaVersion: 1; revision: string; pages: Record<string, ManifestPage> }
export interface PullPreview { scanEpoch: number; revision: string; actions: PullAction[]; remotePages: SyncPage[] }
...
export class SyncRuntime {
  ...
  private async writeBase(revision: string, pages: SyncPage[]): Promise<void> {
    const state: BaseState = { schemaVersion: 1, revision, pages: Object.fromEntries(pages.map((page) => [page.pageId, { pageId: page.pageId, relativePath: page.path, title: page.title, contentHash: page.contentHash }])) };
  ...
  async previewPull(): Promise<PullPreview> {
    ...
    for (const page of remote) { const local = localByPath.get(page.path); const path = `${this.mapping.rootPath}/${page.path}`; if (!local) actions.push({ kind: "create", path, body: page.body }); else if (local.contentHash !== page.contentHash) actions.push({ kind: "write", path, body: page.body }); }
    ...
    return { scanEpoch: scan.scanEpoch, revision: (await this.remote.getHead(this.mapping.spaceId)).revision, actions, remotePages: remote };
  }
  async applyPull(preview: PullPreview): Promise<void> { const tx = new PullTransaction(this.vault, this.control, `${this.root}/pull`); await tx.prepare(preview.actions, preview.scanEpoch); await tx.apply(this.scanEpoch); await this.writeBase(preview.revision, preview.remotePages); this.mapping.status = "active"; }
  async previewPush(): Promise<PushPreview> {
    ...
    for (const file of [...local.added, ...local.modified, ...local.renamed]) { const pageId = file.pageId ?? crypto.randomUUID(); changes.push({ operation: "upsert", pageId, path: file.relativePath, title: file.title, body: file.normalizedBody, contentHash: await contentHash(file.normalizedBody) }); }
  ...
  async applyPush(preview: PushPreview): Promise<void> { const result = await new PushService(this.remote, this.control, `${this.root}/push`).publish({ spaceId: this.mapping.spaceId, baseRevision: preview.revision, changes: preview.changes, capabilities: preview.capabilities }); await this.writeBase(result.revision, await this.remote.snapshot(this.mapping.spaceId)); this.mapping.status = "active"; }
}
