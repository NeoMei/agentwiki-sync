...
export class AgentWikiSyncSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    for (const mapping of this.plugin.settings.mappings) new Setting(this.containerEl).setName(mapping.spaceId).setDesc(`${mapping.rootPath} · ${mapping.status}`).addButton(button=>button.setButtonText(mapping.status==="pending"?"Cancel mapping":"Remove mapping").setWarning().onClick(async()=>{button.setDisabled(true);try{await this.plugin.removeMapping(mapping.spaceId);this.display();}finally{button.setDisabled(false);}}));
    ...
    new Setting(this.containerEl).setName("Disconnect this device").setDesc("Clears the local credential. If offline, revoke the device later in AgentWiki Web.").addButton(button=>button.setButtonText("Disconnect").setWarning().onClick(async()=>{await this.plugin.disconnect();this.display();}));
  }
}
