...
export class PreviewModal extends Modal {
  ...
  onOpen(): void {
    ...
    for(const binding of this.bindings){const setting=new Setting(this.contentEl).setName(`${binding.localPath??"new file"} ↔ ${binding.remotePath}`).setDesc("Choose an optional local page, then choose Local, Remote, or manual content. The remote revision remains the exact base.");setting.addDropdown(dropdown=>{dropdown.addOption("","Create/use remote path");for(const candidate of binding.localCandidates)dropdown.addOption(candidate.path,candidate.path);dropdown.setValue(binding.localPath??"").onChange(value=>{const candidate=binding.localCandidates.find(item=>item.path===value);binding.localPath=candidate?.path??null;binding.localBody=candidate?.body??null;binding.localVaultByteHash=candidate?.vaultByteHash??null;binding.resolution=candidate?null:"remote";});});setting.addDropdown(dropdown=>dropdown.addOption("","Choose…").addOption("local","Keep Local").addOption("remote","Use Remote").addOption("manual","Manual body").setValue(binding.resolution??"").onChange(value=>{binding.resolution=value==="local"||value==="remote"||value==="manual"?value:null;}));setting.addTextArea(text=>text.setPlaceholder("Manual body (used only with Manual body)").onChange(value=>{binding.manualBody=value;}));}
