export function normalizeMarkdown(text: string): string {
  if (text.startsWith("\ufeff")) throw new TypeError("Markdown must not begin with BOM");
  return text.replace(/\r\n?/g, "\n");
}

export function decodeVaultMarkdown(bytes: Uint8Array): { text: string; normalized: string } {
