...
import type { LocalStatus, ManifestPage, MoveHint, ResolvedFile, ScanResult, ScannedFile, VaultFile } from "./model";
import { portablePathKey, titleFromPath, validatePortablePath } from "./portable-path";
...
export async function scanMapping(files: VaultFile[], options: ScanOptions): Promise<ScanResult> {
  if (files.length > options.capabilities.pages) throw new RangeError("SPACE_TOO_LARGE: page count");
  ...
  for (const file of files) {
    ...
    const normalizedBytes = new TextEncoder().encode(decoded.normalized).byteLength;
    bodyBytes += normalizedBytes;
    if (bodyBytes > options.capabilities.bodyBytes) throw new RangeError("SPACE_TOO_LARGE: body bytes");
    scanned.push({ relativePath: path, title: titleFromPath(path), normalizedBody: decoded.normalized, contentHash: await contentHash(decoded.normalized), vaultByteHash: await sha256Hex(file.bytes) });
  }
  const manifestBytes = canonicalBytes({ pages: scanned.map(({ relativePath: path, title, contentHash: hash }, ordinal) => ({ ordinal, path, title, contentHash: hash })) }).byteLength;
  if (manifestBytes > options.capabilities.manifestBytes) throw new RangeError("SPACE_TOO_LARGE: manifest bytes");
  return { complete: options.complete, scanEpoch: options.scanEpoch, files: scanned, bodyBytes, manifestBytes };
}

export function resolvePageIdentities(manifest: Record<string, ManifestPage>, files: ScannedFile[], hints: MoveHint[]): ResolvedFile[] {
  const byPath = new Map(Object.values(manifest).map((page) => [portablePathKey(page.relativePath), page]));
...
export function computeStatus(manifest: Record<string, ManifestPage>, files: ResolvedFile[], scan: ScanResult): LocalStatus {
