...
export class GenerationRepository {
  ...
  async write(
    ...
    for (const [pageId, page] of Object.entries(pages)) {
      ...
      if (body === undefined)
        throw new Error(`Missing base body for ${pageId}`);
      page.contentHash = await contentHash(body);
      bodyBytes += new TextEncoder().encode(body).byteLength;
      await this.store.write(
        this.path(input.generationId, `base/${await opaqueFileKey(pageId)}.md`),
        body,
      );
    }
  ...
  async verify(generationId: string): Promise<SpaceManifest> {
    ...
    if (
      ...
      throw new Error("baseline corrupt: revision metrics mismatch");
    return manifest;
  }
  ...
  async read(
    ...
    for (const page of Object.values(manifest.pages)) {
      ...
      if (body === null) throw new Error("baseline corrupt: missing body");
