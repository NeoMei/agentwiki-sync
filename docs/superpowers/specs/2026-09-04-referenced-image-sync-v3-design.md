...
“保留两份”不猜测页面分流。用户必须选择哪一版继续使用原 `attachmentId` 和现有引用；另一版创建新 ID，插件在预览中提出可移植名，例如 `assets/photo (2).png`，并将用户确认的精确路径绑定到 confirmation hash。服务端在 Finalize 中权威校验并原子接受该名称；若被并发占用则返回 `ATTACHMENT_NAME_CONFLICT`，必须重新预览确认，不得静默加后缀或改变已确认路径。只有用户在预览中显式把某个 Page 引用改向新版本时，系统才改写该 Page。
