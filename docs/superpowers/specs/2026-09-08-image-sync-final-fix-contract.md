...
### 两项剩余本地门的追加授权（2026-09-08）

用户在获知插件重复扫描性能问题、服务端 OpenCode 路径硬编码测试失败之后明确回复“继续”，授权这两项独立修复及审查：

- 插件只消除 inline-code 反引号重复扫描，保留完整等长 run、未闭合字面量、code/comment/fence、源范围、短名与标准相对链接规则；不导入或复制服务端实现。以实际字符访问计数证明增长有界，保留真实 Runtime 引用 ID/未引用图片零读取行为。
- 服务端只把现有 bundled OpenCode 测试改为受控、平台正确的真实解析测试，不依赖工作站恰好安装的二进制路径。不改运行器生产逻辑、依赖、版本或安装布局，不删除断言、不跳过失败用例。验证无配置覆盖时确实选择 fixture 中的 bundled CLI，调用参数和 shell 安全属性保持正确。
- 两项各自提交、各自独立审查；完整插件 check/audit 和专用测试库绑定的完整服务端测试重跑并留存独立 exit 与日志。生产迁移、部署、安装、发布和真实设备测试仍按既有外部门执行，不能用本地通过替代。
