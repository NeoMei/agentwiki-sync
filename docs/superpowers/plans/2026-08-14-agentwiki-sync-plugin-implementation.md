...
- [ ] **Step 2: 运行 `npm test -- tests/unit/release-metadata.test.ts`，确认因工程文件缺失失败。**
- [ ] **Step 3: 创建 strict TypeScript、Vitest、ESLint、esbuild 和最小插件入口；`onload()` 只注册设置页/命令，不联网。**
- [ ] **Step 4: 运行 `npm run check`，预期测试、lint、typecheck、build 全通过。**
- [ ] **Step 5: 提交 `chore: scaffold Obsidian plugin`。**
...
- [ ] **Step 1: 写固定 fixture 的失败测试，断言 `Hello\n` content hash、confirmation hash、batch hash、428 bytes 和 Unicode key 顺序。**
- [ ] **Step 2: 运行三份协议测试，确认函数不存在或 fixture 不匹配。**
- [ ] **Step 3: 用 Web Crypto 与严格 canonical JSON 实现最小协议函数；拒绝 float、undefined、循环和未配对 surrogate。**
- [ ] **Step 4: 写 Zod response schema，响应允许未知字段、request 拒绝未知字段；decimal count 先解析 bigint。**
- [ ] **Step 5: 实现同时受 item/body byte 限制的确定性 batching，并运行全部协议测试。**
- [ ] **Step 6: 提交 `feat: add sync protocol compatibility boundary`。**
...
- [ ] **Step 1: 写 BOM、非法 UTF-8、CRLF/LF、末尾换行、Windows 保留名、NFC/casefold、段/总字节和 `.md` 扩展名失败测试。**
- [ ] **Step 2: 运行测试确认失败原因是缺少校验。**
- [ ] **Step 3: 实现严格 UTF-8 与正文 hash；实现 Unicode NFC、固定 casefold 表所需映射和可移植路径校验。**
- [ ] **Step 4: 增加 `Straße/İ.MD → strasse/i\u0307.md`、COM¹/LPT³ 和 hash 文件键 fixtures 并跑绿。**
- [ ] **Step 5: 提交 `feat: validate portable vault content`。**
...
- [ ] **Step 1: 写不完整扫描不得产生 archive、5,001 页/100 MiB/4 MiB 阻塞、scanEpoch 变化失效测试。**
- [ ] **Step 2: 写 path 优先、move hint、唯一 hash fallback、歧义与跨 Space 新 ID 测试并确认红。**
- [ ] **Step 3: 实现流式扫描和身份解析，只在完整扫描上计算 added/modified/renamed/deleted。**
- [ ] **Step 4: 实现远端 head 与 base 的 clean/local/remote/both 状态，运行测试。**
- [ ] **Step 5: 提交 `feat: compute deterministic sync status`。**
...
- [ ] **Step 1: 写 path/title 真值表、非重叠 diff3、重叠/删除修改/双向移动冲突测试。**
- [ ] **Step 2: 写任一分支 >10,000 行整篇冲突、末尾换行独立合并测试并确认红。**
- [ ] **Step 3: 实现字段合并和 node-diff3 正文合并，冲突 ID 使用内容与范围 hash。**
- [ ] **Step 4: 写首次 Clone、空远端发布、双方内容、显式跨路径绑定和 title-from-stem 测试。**
- [ ] **Step 5: 实现首次绑定 plan，base 永远为固定远端 R，本地差异保持 dirty；运行测试。**
- [ ] **Step 6: 提交 `feat: plan pull merges and initial binding`。**
...
- [ ] **Step 1: 写 current/prev/next 每个故障点、同 generation 异 hash、未知 schema、旧单 key 迁移失败测试。**
- [ ] **Step 2: 实现 canonical payload hash、三副本读取选择和串行写入，确认普通 payload 测试通过。**
- [ ] **Step 3: 写 pointer phase×candidate、active=false tombstone、不可变 base/manifest 损坏测试。**
- [ ] **Step 4: 实现 generation 写入/重读、base 三指标重算、journal-aware pointer 收敛和最近两代保留。**
- [ ] **Step 5: 提交 `feat: persist crash-safe sync generations`。**
...
- [ ] **Step 1: 写 create/write/rename/trash、A↔B、三路径环、目录创建和回收站失败测试。**
- [ ] **Step 2: 对 prepared/applying/committing/committed/rollback 每个持久化点注入退出并确认红。**
- [ ] **Step 3: 实现 results/snapshots staging、条件 `Vault.process`、临时路径置换与逐步 journal。**
- [ ] **Step 4: 实现唯一状态推断、rollback、pointer tombstone 与 failed freeze；运行故障矩阵。**
- [ ] **Step 5: 提交 `feat: apply crash-safe pull transactions`。**
...
- [ ] **Step 1: 写 HTTPS/loopback URL、3xx 拒绝、错误 envelope、分页固定指标与授权 header 测试。**
- [ ] **Step 2: 写 exchange 每个崩溃点、response lost、collision 换 credential、过期和 revoke 测试。**
- [ ] **Step 3: 实现 requestUrl client、端点级重试预算和无正文诊断。**
- [ ] **Step 4: 实现随机 Secret ID、connection journal、credential_stored 提交顺序、activate 和恢复。**
- [ ] **Step 5: 运行连接/客户端测试，扫描代码确认 secret 不进入 Control Store/log。**
- [ ] **Step 6: 提交 `feat: connect human AgentWiki devices`。**
...
- [ ] **Step 1: 写远端领先禁止 Push、preview capability hash、confirmation 限制和 payload snapshot 测试。**
- [ ] **Step 2: 写批次响应丢失、finalize 响应丢失、noop、BASE_STALE、session expiry 和 credential rotation 测试。**
- [ ] **Step 3: 实现 JSONL sidecar、payload、batch index、receipt 与精确临时空间估算。**
- [ ] **Step 4: 实现 create/upload/finalize/query 状态机及 published 后本地 generation 幂等提交。**
- [ ] **Step 5: 运行 Push 故障矩阵，确认用户确认后的新编辑仍被 Status 判为 modified。**
- [ ] **Step 6: 提交 `feat: publish confirmed vault changes`。**
...
- [ ] **Step 1: 写双 Space 隔离、并发拒绝、pending 映射隐藏、server/vault identity mismatch freeze 测试。**
- [ ] **Step 2: 写添加/取消/移除映射、断开、离线忘记和新 Vault 物理副本确认测试。**
- [ ] **Step 3: 实现服务编排、取消边界、每批/50 文件 yield 与进度事件。**
- [ ] **Step 4: 运行集成测试并提交 `feat: orchestrate manual space sync`。**
...
- [ ] **Step 1: 写插件加载零网络、命令注册、活动文件选 Space 和按钮能力 view-model 测试。**
- [ ] **Step 2: 写设置页内联连接状态、无自动浏览器跳转、预览确认/取消边界测试。**
- [ ] **Step 3: 实现原生 `PluginSettingTab/Modal/Setting` UI，不引入 React。**
- [ ] **Step 4: 实现诊断预览与脱敏、100 项文件分页、冲突选择和最终结果编辑。**
- [ ] **Step 5: 运行 UI/lifecycle 测试并提交 `feat: add native Obsidian sync UI`。**
...
- [ ] **Step 1: 写桌面 Push→移动 Pull→移动 Push→桌面 Pull 的失败 E2E。**
- [ ] **Step 2: 实现 fake remote 的 revision、Snapshot/Delta、角色、session、批次与原子 finalize。**
- [ ] **Step 3: 覆盖双设备非重叠合并、重叠冲突、归档、角色降级、响应丢失和 Relation-only revision。**
- [ ] **Step 4: 对所有 journal/HTTP fault point 参数化执行，运行 `npm test -- tests/e2e tests/integration`。**
- [ ] **Step 5: 提交 `test: verify end-to-end manual sync flows`。**
...
- [ ] **Step 1: 写 5,000 页/100 MiB 流式 scan、分页 Pull、分批 Push 与 10,001 行降级性能测试。**
- [ ] **Step 2: 写 bundle 扫描，拒绝 `node:`, `fs`, `child_process`, `FileSystemAdapter` 和 secret fixture。**
- [ ] **Step 3: 编写安装、连接、首次绑定、恢复、隐私、已知上游依赖文档。**
- [ ] **Step 4: 运行 `npm ci && npm run check`，随后在干净临时 Vault 执行 plugin load smoke test。**
- [ ] **Step 5: 生成验证报告，记录真实 AgentWiki API/协议包尚未发布，因此 live 联调为外部待办而非插件缺失。**
- [ ] **Step 6: 提交 `release: prepare AgentWiki Sync v1`。**
