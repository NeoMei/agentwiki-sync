...
## 2026-09-05 续作与发布修订（优先于下方旧版示例）

- 协议正式依赖统一为已发布的 `@neomei/agentwiki-sync-protocol@0.5.1`；下文 `0.5.0` 是过期候选值，不能再用于安装、发布或验收。已发布 0.5.1 的 tarball、ESM/CJS、类型和完整性均已核验。
- Tasks 1–9 与 Task 10 本地候选已完成，Task 10 协议包发布已完成；按本计划的独立 SDD ledger 和提交证据续作，不因文中旧 checkbox 重做已完成任务。
- 用户已授权补齐插件、整合服务端、部署、双端验收与 GitHub 正式发布。生产前仍必须具备配对备份、迁移与回滚验证；授权不替代验收，也不允许未经确认写入用户主 Vault。
- 本地插件 Task 11–18 可在已发布协议基础上推进，不必等待服务端生产部署；但正式插件发布和真实生产验收仍严格在服务端更新及其验证之后。
- 服务端候选 `2b0b49d` 与远端主线 `711cae7` 已分叉。整合必须保留主线后续修复并重新跑完整门，禁止直接以旧候选覆盖当前生产或回退主线。
- 插件正式 tag 使用 `0.4.0`，不带 `v`：当前 `.github/workflows/release.yml` 只监听纯数字语义版本，并要求 tag 与 manifest version 完全一致；下方 `v0.4.0` 示例不适用。
- 真实移动端验收为单独证据门；无移动设备时标记未完成，不能用桌面窄屏或 mock 冒充。不得在未满足正式发布门时将半成品称为完整图片同步版本。
...
`BlobRequirementV3Schema` 只接受上述五个字段：`sizeBytes` 为大于 0 且不超过 10 MiB 的 canonical decimal string，MIME/尺寸/解码像素与 `SyncAttachmentV3` 使用同一硬上限；任何非法 decimal 都必须作为普通 Zod failure 返回，不能从 `safeParse()` 抛原生转换异常。create request 必须拒绝旧 `contentHashes` 和任何未知字段；`blobRequirements` 按 `contentHash` 严格升序且唯一，最多 1,000 项。`attachmentCount` 精确表示 confirmation changes 中 `upsert_attachment` 的数量，不是最终 Revision 的 Attachment 总数；必须满足 `attachmentCount <= changeCount`。同一 Blob 可被多个 Attachment identity 复用，因此只要求 `blobRequirements.length <= attachmentCount`；`attachmentCount = 0` 时 requirements 必须为空，`attachmentCount > 0` 时必须非空。`transferBlobBytes` 等于去重 requirements 的 `sizeBytes` 精确总和并且不得超过 100 MiB。requirements 必须包含 confirmation 中每个 `upsert_attachment.attachment.contentHash` 的唯一集合，包括服务端可能已有的 Blob；schema 无法看到 confirmation preimage，Task 7 必须在 batch upload 与 Finalize 交叉验证这一集合。现有 confirmation manifest/hash、五个 canonical digest 和错误码集合不变。
